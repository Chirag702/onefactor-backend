...
public class GroupController {
    ...
    @PostMapping
    public ResponseEntity<?> createGroup(@RequestBody Groups groups, @RequestHeader("Authorization") String tokenHeader) {
        try {
            ...
            return new ResponseEntity<>("An error occurred while creating the groups", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<Groups> getGroupById(@PathVariable Long id) {
        Groups groups = groupService.getGroupById(id);
        return new ResponseEntity<>(groups, HttpStatus.OK);
    }
